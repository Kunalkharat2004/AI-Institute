...
const pages = [
  ...
  {
    name: "Handbook",
    path: "https://aicte-india.org/sites/default/files/approval/APH%20Final.pdf",
  },
  { name: "Inspection", path: "/inspection/timeline/form" },
  { name: "FAQ", path: "/faq" },
  {
    name: "AI Assistant",
    path: "https://cdn.botpress.cloud/webchat/v2.2/shareable.html?configUrl=https://files.bpcontent.cloud/2024/12/08/13/20241208135325-9430ATBC.json",
  },
...
const Navbar = () => {
  ...
  const { token, setToken, setHasfetchData, setPaymentStatus } = useTokenStore(
    (state) => state
  );
  React.useEffect(() => {
    AOS.init({
      ...
      <Container maxWidth="2xl">
        <Toolbar disableGutters sx={{ justifyContent: "space-between" }}>
          ...
          >
            ...
            >
              <img src={SIHLogo} alt="SIH Logo" className="h-8 md:h-4 w-auto" />
              <img
                src={DigitalIndiaLogo}
                alt="CGWB Logo"
                className="h-8 md:h-10 w-auto"
          ...
          <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                ...
                >
                  {user?.email?.length > 0 ? user.email[0].toUpperCase() : "U"}
                </Avatar>
              </IconButton>
            </Tooltip>
            <Menu
